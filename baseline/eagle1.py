...
def predict(tokenizer, input_data, model,task_name):
    ...
    openai.base_url = "http://0.0.0.0:8030/v1/" 
    models = openai.models.list()
    model = models.data[0].id 
    response = openai.chat.completions.create(
        model=model,   # e.g. "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B"
        messages=messages,
        temperature=0.6,
        top_p=0.95,
        max_tokens=14000,
...
if __name__ == "__main__":
    ...
    base_dir = f'../vllm_h100_DeepSeek-R1-Distill-Qwen-1.5B_{args.dataset}_seed{args.seed}/'
