...
def predict(input_data, model,temperature):
    ...
    checking_outputs = model.generate(
        [input_data], sampling_params=sampling_params, return_hidden_states=True
    )
    # print('checking_output',checking_output)
        ...
        for i in range(num_generations + 1):
            ...
            try:
                if i == 0:
                    ...
                    # print('most_real_answer',most_real_answer)
                else:
                    # print('----22222----------')
                    (
                        real_answer,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="deepseek-ai/DeepSeek-R1-Distill-Qwen-32B", help="model name")
    parser.add_argument("--start", type=int, help="dataset", default=57)
    parser.add_argument("--end", type=int, help="dataset", default=107)  #
    ...
    if args.model == "Qwen/QwQ-32B":
        ...
        llm = sgl.Engine( model_path="/home/original_models/DeepSeek-R1-Distill-Qwen-1.5B", enable_return_hidden_states=True,mem_fraction_static=0.2)
    elif args.model == "deepseek-ai/DeepSeek-R1-Distill-Qwen-32B":
        llm = sgl.Engine( model_path="/home/original_models/QwQ-32B", enable_return_hidden_states=True,mem_fraction_static=0.9)

    inference_model_pickle(task_name='math', base_dir=args.base_dir,
                           start=args.start, end=args.end,model=llm)
    print("done")
