...
def main(dataset,method,data_dir,model_name):
    ...
    for number in tqdm(range(start, end)):
        ...
        x, y = getting_training_examples(pkl_path,method)
        X.extend(x)
        Y.extend(y)
    print("X",len(X))
    ...
    history = train_probe_regression(
        model, train_loader, val_loader, epochs=100, lr=1e-3,
        device='cuda', early_stop_rounds=20, save_pred_path=f's1_valid_new_{model_name}_{dataset}_{method}_val_pred_results.npz',method=method,dataset_name = dataset,
    ...
    np.savez(f's1_valid_new_{model_name}_{dataset}_{method}_test_pred_results.npz', pred=all_preds, target=all_targets)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str,  help="model",default="1.5b")
    parser.add_argument("--method", type=str,  help="method for X",default='last_hidden_state')
    parser.add_argument("--data_dir", type=str,  help="method for X",default='/shared_workspace_mfs/ximing/data_s1_200_segments_math_small')
