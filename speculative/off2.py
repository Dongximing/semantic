...
def speculative_decoding(llm_big,llm_small,target_tokenizer,speculative_tokenizer,problem,max_new_tokens,model_target_probe,model_spec_probe):
        ...
        while checking_is_finish(generated_text,max_new_tokens,use_target):
            ...
            if not begin:
                ...
                speculative_outputs = llm_small.generate([small_input], sampling_params=sampling_params, return_hidden_states=True,return_logprob=True)
                prob_small_result = [
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seed", type=int, help="seed", default=987)
   ...
   tp_size=1 
    ...
    if args.seed == 987:
        wrong_list =  [100, 101, 108, 110, 115, 120, 123, 135, 138, 145, 158, 160, 168, 198, 204, 205, 214, 217, 222, 228, 240, 242, 248, 264, 282, 284, 285, 286, 292, 294, 295, 298, 302, 306, 308, 324, 340, 355, 369, 379, 383, 392, 394, 400, 401, 403, 419, 422, 425, 456, 460, 470, 481, 486, 490, 494]
    elif args.seed == 321:
        wrong_list =[100, 126,138, 158, 168, 189, 198, 204, 205, 210, 213,  217, 219, 222, 240, 242, 246, 264, 284, 286, 295, 298, 306, 308,  324, 340, 349, 351, 352, 355, 381, 383, 392,  403, 419, 422, 425, 435, 448, 456, 460, 481, 485, 486, 490, 494, 497]
    elif args.seed == 654:
        wrong_list = [100, 101, 129, 152,  166, 168, 198, 204, 205, 217, 222, 224, 227, 232, 239, 240, 246, 248, 264, 284, 286, 295, 298, 302, 306, 308, 324, 327, 333, 340, 349, 351, 358, 380, 381, 383, 392, 400, 403, 409, 419, 421, 422, 432,  456, 460, 466, 478, 481, 483, 490, 491, 498]
