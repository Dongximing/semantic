        ...
        while checking_is_finish(generated_text,max_new_tokens,use_target):
            ...
            if not begin:
                ...
                if not keep_doing:                
                    ...
                    pooling_batch = torch.stack(pooling_list).float().to('cuda:5')  # shape: [3, hidden_dim]
                    target_batch = torch.stack(target_pooling_list).float().to('cuda:5')  # 同上
                    with torch.no_grad():
                        prob_targets = model_target_probe(target_batch.float().to(f"cuda:{5}"))
                        prob_specs = model_spec_probe(pooling_batch.float().to(f"cuda:{5}"))
...
if __name__ == "__main__":
   ...
   tp_size=1 
    ...
    model_spec_probe = model_spec_probe. 
