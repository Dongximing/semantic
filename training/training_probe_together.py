...
def train_probe_regression(
    ...
    for epoch in range(1, epochs+1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), f'{method}_aime_math_best_probe_mse.pt')
        else:
...
def main(method):
    data_dir_aime = '/data/ximing/aime'
    data_dir_math_500 = '/data/ximing/math-result_left'
    start = 0
    end = 60
    X, Y = [], []
    base_dir = data_dir_aime
    ...
    history = train_probe_regression(
        model, train_loader, val_loader, epochs=50, lr=1e-3,
        device='cuda:1', early_stop_rounds=7, save_pred_path=f'{method}_aime_math_val_pred_results.npz',method=method
    )


    model.load_state_dict(torch.load(f'{method}_aime_math_best_probe_mse.pt'))
    model.eval()
    ...
    np.savez(f'{method}_aime_math_test_pred_results.npz', pred=all_preds, target=all_targets)
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.title(f'{method}_aime_math_Validation Loss Curve')
    plt.savefig(f'{method}_aime_math_validation_loss_curve.png',dpi=200,bbox_inches='tight')
    plt.show()
#/data/ximing/aime
